...
flowchart TB
    ...
    reg -->|"_create_views()"| bq_views
    reg -->|"_create_feature_group()"| fs_registry
    reg -->|"_create_feature()"| fs_registry
    bq_table --> bq_views
    bq_views --> fs_registry

    read -->|"_get_feature_group()"| fs_registry
    read -->|"_list_features()"| fs_registry
    fs_registry -->|"BigQuery source URI<br/>Feature IDs"| read
    read -->|"_build_sql()"| bq_read
    bq_views --> bq_read
    bq_read --> read
    read --> stdout
    read --> gcs
...
flowchart TB
    run["register.run()"]
    auth["access_token()"]
    create_views["_create_views(client, project_id)"]
    create_fg["_create_feature_group(project_id, region, token, fg_id, view_id)"]
    create_feature["_create_feature(project_id, region, token, fg_id, feature_id, desc)"]
    request["_request(url, method, token, payload)"]
    poll["_poll_lro(operation_name, token, region)"]

    bq_client["bigquery.Client"]
    bq_view_a["BQ View<br/>v_property_features_emb_a"]
    bq_view_b["BQ View<br/>v_property_features_emb_b"]
    rest_fg["Vertex AI REST<br/>POST /featureGroups"]
    rest_feature["Vertex AI REST<br/>POST /featureGroups/{fg}/features"]
    rest_lro["Vertex AI REST<br/>GET /operations/{operation}"]
...
- `_create_feature_group()` は View の `bq://...` URI を Feature Group に登録する。
- `_create_feature()` は Feature Group 配下に column metadata を登録する。
...
flowchart TB
    ...
    auth["access_token()"]
    get_fg["_get_feature_group(project, region, token, fg_id)"]
    list_features["_list_features(project, region, token, fg_id)"]
    request["_request(url, token)"]
    bq_ref["_bq_ref(input_uri)"]
    build_sql["_build_sql(bq_ref, entity_id_cols, feature_ids)"]
    fetch["_fetch(bigquery.Client, sql, cols)"]
    log_rows["_log_rows(rows, source)"]
    write_jsonl["_write_jsonl(rows, local_path)"]
    upload["_upload_gcs(project_id, bucket_name, local_path, blob_name)"]

    fs_get["Vertex AI REST<br/>GET /featureGroups/{fg_id}"]
    fs_list["Vertex AI REST<br/>GET /featureGroups/{fg_id}/features"]
    bq_view["BigQuery View<br/>from bigQuerySource.inputUri"]
    bq_api["BigQuery API<br/>client.query(sql)"]
    local_file["/tmp/result_{source}_{YYYYMMDD}.jsonl"]
    gcs["GCS<br/>offline-batch/{YYYYMMDD}/{source}/result.jsonl"]
...
- BigQuery View 名は `_get_feature_group()` の戻り値から得る。
- SELECT column は `_list_features()` の戻り値から得る。
